...
void broadcast_centroids(const double* centroids, int K, int D, int root, MPI_Comm comm) {
    void * buffer = (void*)centroids; // explicit cast stream
    MPI_Bcast(buffer, K*D, MPI_DOUBLE, root, comm);
}
                      ...
                      MPI_Comm      comm)
{
    ...
    MPI_Reduce(local_cent, accum.data(), K*D, MPI_DOUBLE, MPI_SUM, 0, comm);
    MPI_Reduce(local_cnts, cnts.data(),    K,   MPI_INT,    MPI_SUM, 0, comm);
                     ...
                     MPI_Comm      comm)
{
    ...
    MPI_Reduce(centroids, recv_cent.data(), K*D, MPI_DOUBLE, MPI_SUM, server_rank, comm);
    MPI_Reduce(local_cnts, recv_cnts.data(),    K, MPI_INT,    MPI_SUM, server_rank, comm);
    ...
    MPI_Bcast(recv_cent.data(), K*D, MPI_DOUBLE, server_rank, comm);
    // Replace local buffer
    std::memcpy(centroids, recv_cent.data(), sizeof(double)*K*D);
}

void allreduce_average(double* centroids, const int* counts, int K, int D, MPI_Comm comm) {
    // accumulators
    std::vector<double> sum_cent(K*D);
    std::vector<int>    sum_cnt(K);

    // compute local weighted-sum in centroids itself and share counts
    MPI_Allreduce(centroids, sum_cent.data(), K*D, MPI_DOUBLE, MPI_SUM, comm);
    MPI_Allreduce(counts,    sum_cnt.data(),  K,   MPI_INT,    MPI_SUM, comm);
