...
__device__ RGBColorF colorf_multiply(RGBColorF x, RGBColorF y)
{
    return RGBColorF{
        x.r * y.r,
        x.g * y.g,
...
__device__ RGBColorF tex_solidColorValue(const SolidColor * restrict t){
    return t->color;
}

__device__ RGBColorF tex_checkerValue(const Checker * restrict c,
...
__device__ RGBColorF tex_value(const Texture * restrict t, CFLOAT u, CFLOAT v, vec3 p){
...
__device__ RGBColorF tex_imageValue(const Image * restrict img,
        CFLOAT u, CFLOAT v){
