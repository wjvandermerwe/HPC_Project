...
__device__ static vec3 util_vec3Refract(vec3 uv, vec3 n, CFLOAT etai_over_etat){
...
__device__ static bool mat_metalScatter(const MetalMat * restrict nmetalMat,
...
__device__ static bool mat_lambScatter(const LambertianMat * restrict nlambMat,
                            const HitRecord * restrict rec,
                            RGBColorF * restrict attenuation, Ray * restrict out){
...
__device__ static CFLOAT reflectance(CFLOAT cosine, CFLOAT ref_idx){
...
__device__ static bool mat_dielectricScatter(const DielectricMat * restrict ndielectricMat, const Ray * restrict rayIn, const HitRecord * restrict rec, RGBColorF * restrict attenuation, Ray * restrict out ){

    *attenuation = makeColorF(1.0,1.0,1.0);
...
__device__ bool mat_scatter (const Ray * restrict rayIn, const HitRecord * restrict rec, RGBColorF * restrict attenuation, Ray * restrict out){
